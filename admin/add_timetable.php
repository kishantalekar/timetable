        ...
        for ($i = 0; $i < 5; $i++) {
            ...
            for ($j = 0; $j < 6; $j++) {
                if ($weekTimeTable[$i][$j]['type'] === 'Lab') {
                    ...
                    $insertQuery = "INSERT INTO timeschedule (department_name, semester_name, subject_name, time, date, teacher_id) VALUES ('$branch', '$semester', '".$weekTimeTable[$i][$j]['subject_name']."', '".$weekTimeTable[$i][$j]['time']."', '".$weekDays[$i]."', ".$weekTimeTable[$i][$j]['teacher_id'].")";
                ...
                if ($j === 3) {
                    echo "<th class='text-center'><b style='text-size=24'>".$lunch."</b></th>";
                }
            }
            echo "</tr>";
        }
    } else {
        echo "<div style='font-size: 28'><b>Not enough data for selected Course and semester.</b></div>";
    }
}
